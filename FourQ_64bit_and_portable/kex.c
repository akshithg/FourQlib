...
/*************** BPV-ECDH USING COMPRESSED PUBLIC KEYS ***************/

ECCRYPTO_STATUS CompressedBPVPrecalculation(unsigned char *SecretKey, unsigned char *PublicKey)
{
	ECCRYPTO_STATUS Status = ECCRYPTO_ERROR_UNKNOWN;
	Status = RandomBytesFunction(SecretKey, 32 * BPV_n);
	if (Status != ECCRYPTO_SUCCESS)
	{
		goto cleanup;
	}
	...
	point_t P;
	int k;
	unsigned char secretTemp[32], publicTemp[32];

	for (k = 0; k < BPV_n; k++)
	{
		memmove(secretTemp, SecretKey + k * 32, 32);
		ecc_mul_fixed((digit_t *)secretTemp, P);
		encode(P, publicTemp);
		memmove(PublicKey + k * 32, publicTemp, 32);
	}
...
ECCRYPTO_STATUS CompressedBPVKeyGen(const unsigned char *SecretKey, const unsigned char *PublicKey, unsigned char *lastSecret, unsigned char *lastPublic)
{
	ECCRYPTO_STATUS Status = ECCRYPTO_ERROR_UNKNOWN;
	...
	memmove(publicTemp, PublicKey + 32 * index[0], 32);

	Status = decode(publicTemp, Added);
	if (Status != ECCRYPTO_SUCCESS)
	{
		goto cleanup;
	}

	point_setup(Added, AddedExtproj);

	index[1] = BPV_n * index[1] / 256;
	memmove(secretTemp2, SecretKey + 32 * index[1], 32);
	memmove(publicTemp, PublicKey + 32 * index[1], 32);

	Status = decode(publicTemp, Temp);
	if (Status != ECCRYPTO_SUCCESS)
	{
		goto cleanup;
	}
	...
	for (k = 0; k < BPV_k - 2; k++)
	{
		...
		memmove(publicTemp, PublicKey + 32 * index[k + 2], 32);
		Status = decode(publicTemp, Temp);
		if (Status != ECCRYPTO_SUCCESS)
		{
			goto cleanup;
...
/*************** BPV-ECDH USING UNCOMPRESSED PUBLIC KEYS ***************/

ECCRYPTO_STATUS UncompressedBPVPrecalculation(unsigned char *SecretKey, unsigned char *PublicKey)
{
	ECCRYPTO_STATUS Status = ECCRYPTO_ERROR_UNKNOWN;
	Status = RandomBytesFunction(SecretKey, 32 * BPV_n);
	if (Status != ECCRYPTO_SUCCESS)
	...
	{
		memmove(secretTemp, SecretKey + k * 32, 32);
		ecc_mul_fixed((digit_t *)secretTemp, (point_affine *)publicTemp);
		//encode(P, publicTemp);
		memmove(PublicKey + k * 64, publicTemp, 64);
	}
...
ECCRYPTO_STATUS UncompressedBPVKeyGen(const unsigned char *SecretKey, const unsigned char *PublicKey, unsigned char *lastSecret, unsigned char *lastPublic)
{
	ECCRYPTO_STATUS Status = ECCRYPTO_ERROR_UNKNOWN;
	digit_t *S = (digit_t *)(lastSecret);
	unsigned char *index;
